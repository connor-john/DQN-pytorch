...
@author: Connor
Dueling Agent to handle all learning
"""
...
# Agent to manage learning, networks and dueling
class DuelingAgent():
    ...
    def learn(self):
        ...
        # Dueling learn requirements
        s_val, s_adv = self.q.forward(states)
        s1_val, s1_adv = self.q_next.forward(states_)
        
        q_pred = torch.add(s_val, (s_adv - s_adv.mean(dim = 1, keepdim = True)))[index, actions]
        q_next = torch.add(s1_val, (s1_adv - s1_adv.mean(dim = 1, keepdim = True))).max(dim = 1)[0]
        q_next[dones] = 0.0
